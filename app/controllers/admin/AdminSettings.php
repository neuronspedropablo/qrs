...
class AdminSettings extends Controller {
    ...
    public function main() {
        ...
        if(!empty($_POST)) {
            ...
            $image = [
                'logo_light' => !empty($_FILES['logo_light']['name']) && !isset($_POST['logo_light_remove']),
                'logo_dark' => !empty($_FILES['logo_dark']['name']) && !isset($_POST['logo_dark_remove']),
                'logo_email' => !empty($_FILES['logo_email']['name']) && !isset($_POST['logo_email_remove']),
            ...
            foreach(['logo_light', 'logo_dark', 'logo_email', 'favicon', 'opengraph'] as $image_key) {
                if($image[$image_key]) {
                    ...
                    if(!\Altum\Plugin::is_active('offload') || (\Altum\Plugin::is_active('offload') && !settings()->offload->uploads_url)) {
                        if(!is_writable(UPLOADS_PATH . 'main/')) {
                            Alerts::add_error(sprintf(l('global.error_message.directory_not_writable'), UPLOADS_PATH . 'main/'));
                        }
                    }
                    ...
                    if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                        ...
                        if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                            try {
                                ...
                                $s3->deleteObject([
                                    ...
                                    'Key' => 'uploads/main/' . settings()->main->{$image_key},
                                ]);
                        ...
                        /* Local uploading */
                            ...
                            /* Delete current image */
                            if(!empty(settings()->main->{$image_key}) && file_exists(UPLOADS_PATH . 'main/' . settings()->main->{$image_key})) {
                                unlink(UPLOADS_PATH . 'main/' . settings()->main->{$image_key});
                            }

                            /* Upload the original */
                            move_uploaded_file($file_temp, UPLOADS_PATH . 'main/' . $image_new_name);
                        }

                        /* Database query */
                        settings()->main->{$image_key} = $image_new_name;
                    }
                }
