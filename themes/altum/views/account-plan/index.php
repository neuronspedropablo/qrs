...
<div class="container">
    ...
    <?php if($this->user->plan_id != 'free' && $this->user->payment_subscription_id): ?>
        ...
        <div class="card">
            <div class="card-body">
                <a href="<?= url('account-plan/cancel_subscription' . \Altum\Middlewares\Csrf::get_url_query()) ?>" class="btn btn-block btn-outline-secondary" onclick='return confirm(<?= json_encode(l('account_plan.cancel.confirm_message')) ?>)'><?= l('account_plan.cancel.cancel') ?></a>
            </div>
        </div>
    <?php endif ?>
</div>
